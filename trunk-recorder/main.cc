...
std::vector<float> design_filter(double interpolation, double deci) {
  ...
  std::vector<float> result = gr::filter::firdes::low_pass(
      interpolation, 1, mid_transition_band / interpolation,
      trans_width / interpolation, gr::filter::firdes::WIN_KAISER, beta);
...
void set_logging_level(std::string log_level) {
  ...
  if (log_level == "trace")
    ...
    BOOST_LOG_TRIVIAL(error)
        << "set_logging_level: Unknown logging level: " << log_level;
    return;
  }

  logging::core::get()->set_filter(logging::trivial::severity >= sev_level
...
bool load_config(string config_file) {
  ...
  try {
    ...
    if (config_ver < 2) {
      ...
      BOOST_LOG_TRIVIAL(info)
          << "Modulation type, Squelch and audio levels are now set in each "
             "System instead of under a Source.";
      BOOST_LOG_TRIVIAL(info)
          << "See sample config files in the /example folder and look at "
             "readme.md for more details.";
      BOOST_LOG_TRIVIAL(info) << "After you have made these updates, make sure "
                                 "you add \"ver\": 2, to the top.\n\n";
      return false;
    }
    ...
    std::string frequencyFormatString =
        pt.get<std::string>("frequencyFormat", "exp");
    ...
    config.debug_recorder_address =
        pt.get<std::string>("debugRecorderAddress", "127.0.0.1");
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type &node,
      ...
      BOOST_LOG_TRIVIAL(info) << "\n\nSystem Number: " << sys_count
                              << "\n-------------------------------------\n";
      system->set_short_name(
          node.second.get<std::string>("shortName", default_script.str()));
      ...
      if (system->get_system_type() == "conventional") {
        ...
        BOOST_FOREACH (boost::property_tree::ptree::value_type &sub_node,
                       node.second.get_child("channels")) {
        ...
        if (node.second.count("alphatags") != 0) {
          int alphaIndex = 1;
          BOOST_FOREACH (boost::property_tree::ptree::value_type &sub_node,
                         node.second.get_child("alphatags")) {
        ...
        system->set_delaycreateoutput(
            node.second.get<bool>("delayCreateOutput", false));
        BOOST_LOG_TRIVIAL(info)
            << "delayCreateOutput: " << system->get_delaycreateoutput();

      } else if ((system->get_system_type() == "smartnet") ||
                 (system->get_system_type() == "p25")) {
      ...
      boost::optional<std::string> mod_exists =
          node.second.get_optional<std::string>("modulation");
      ...
      if (mod_exists) {
        ...
        if (boost::iequals(system_modulation, "qpsk")) {
          ...
          qpsk_mod = true;
          BOOST_LOG_TRIVIAL(error) << "! System Modulation Not Specified, "
                                      "could be fsk4 or qpsk, assuming qpsk";
        }
      } else {
        qpsk_mod = true;
      }
      ...
      BOOST_LOG_TRIVIAL(info) << "Analog Recorder Maximum Deviation: "
                              << node.second.get<int>("maxDev", 4000);
      BOOST_LOG_TRIVIAL(info) << "Filter Width: " << filter_width;
      BOOST_LOG_TRIVIAL(info)
          << "Squelch: " << node.second.get<double>("squelch", -160);
      system->set_api_key(node.second.get<std::string>("apiKey", ""));
      BOOST_LOG_TRIVIAL(info) << "API Key: " << system->get_api_key();
      system->set_bcfy_api_key(
          node.second.get<std::string>("broadcastifyApiKey", ""));
      BOOST_LOG_TRIVIAL(info)
          << "Broadcastify API Key: " << system->get_bcfy_api_key();
      system->set_bcfy_system_id(
          node.second.get<int>("broadcastifySystemId", 0));
      BOOST_LOG_TRIVIAL(info)
          << "Broadcastify Calls System ID: " << system->get_bcfy_system_id();
      system->set_upload_script(
          node.second.get<std::string>("uploadScript", ""));
      ...
      BOOST_LOG_TRIVIAL(info)
          << "Audio Archive: " << system->get_audio_archive();
      system->set_talkgroups_file(
          node.second.get<std::string>("talkgroupsFile", ""));
      BOOST_LOG_TRIVIAL(info)
          << "Talkgroups File: " << system->get_talkgroups_file();
      system->set_unit_tags_file(
          node.second.get<std::string>("unitTagsFile", ""));
      BOOST_LOG_TRIVIAL(info)
          << "Unit Tags File: " << system->get_unit_tags_file();
      system->set_record_unknown(node.second.get<bool>("recordUnknown", true));
      BOOST_LOG_TRIVIAL(info)
          << "Record Unknown Talkgroups: " << system->get_record_unknown();
      ...
      BOOST_LOG_TRIVIAL(info)
          << "Decode FSync: " << system->get_fsync_enabled();
      ...
      std::string talkgroup_display_format_string =
          node.second.get<std::string>("talkgroupDisplayFormat", "Id");
      if (boost::iequals(talkgroup_display_format_string, "id_tag")) {
        system->set_talkgroup_display_format(
            System::talkGroupDisplayFormat_id_tag);
      } else if (boost::iequals(talkgroup_display_format_string, "tag_id")) {
        system->set_talkgroup_display_format(
            System::talkGroupDisplayFormat_tag_id);
      } else {
        system->set_talkgroup_display_format(System::talkGroupDisplayFormat_id);
      }
      BOOST_LOG_TRIVIAL(info)
          << "Talkgroup Display Format: " << talkgroup_display_format_string;
      ...
      system->set_bandplan(
          node.second.get<std::string>("bandplan", "800_standard"));
      ...
      system->set_bandplan_spacing(
          node.second.get<double>("bandplanSpacing", 0.0));
      system->set_bandplan_offset(node.second.get<int>("bandplanOffset", 0));

      if (system->get_system_type() == "smartnet") {
        BOOST_LOG_TRIVIAL(info)
            << "Smartnet bandplan: " << system->get_bandplan();
        BOOST_LOG_TRIVIAL(info) << "Smartnet band: " << system->get_bandfreq();

        if (system->get_bandplan_base() || system->get_bandplan_spacing() ||
            system->get_bandplan_offset()) {
          BOOST_LOG_TRIVIAL(info)
              << "Smartnet bandplan base freq: " << system->get_bandplan_base();
          BOOST_LOG_TRIVIAL(info)
              << "Smartnet bandplan high freq: " << system->get_bandplan_high();
          BOOST_LOG_TRIVIAL(info) << "Smartnet bandplan spacing: "
                                  << system->get_bandplan_spacing();
          BOOST_LOG_TRIVIAL(info)
              << "Smartnet bandplan offset: " << system->get_bandplan_offset();
        }
      }

      system->set_hideEncrypted(
          node.second.get<bool>("hideEncrypted", system->get_hideEncrypted()));
      BOOST_LOG_TRIVIAL(info)
          << "Hide Encrypted Talkgroups: " << system->get_hideEncrypted();
      system->set_hideUnknown(node.second.get<bool>("hideUnknownTalkgroups",
                                                    system->get_hideUnknown()));
      BOOST_LOG_TRIVIAL(info)
          << "Hide Unknown Talkgroups: " << system->get_hideUnknown();
      system->set_min_duration(node.second.get<double>("minDuration", 0));
      BOOST_LOG_TRIVIAL(info) << "Minimum Call Duration (in seconds): "
                              << system->get_min_duration();
      system->set_max_duration(node.second.get<double>("maxDuration", 0));
      BOOST_LOG_TRIVIAL(info) << "Maximum Call Duration (in seconds): "
                              << system->get_max_duration();
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type &node,
      ...
      if ((driver != "osmosdr") && (driver != "usrp")) {
        BOOST_LOG_TRIVIAL(error) << "Driver specified in config.json not "
                                    "recognized, needs to be osmosdr or usrp";
      }

      std::string device = node.second.get<std::string>("device", "");
      BOOST_LOG_TRIVIAL(info)
          << "Driver: " << node.second.get<std::string>("driver", "");
      BOOST_LOG_TRIVIAL(info)
          << "Center: " << FormatFreq(node.second.get<double>("center", 0));
      BOOST_LOG_TRIVIAL(info)
          << "Rate: " << FormatSamplingRate(node.second.get<double>("rate", 0));
      BOOST_LOG_TRIVIAL(info)
          << "Error: " << node.second.get<double>("error", 0);
      BOOST_LOG_TRIVIAL(info)
          << "PPM Error: " << node.second.get<double>("ppm", 0);
      BOOST_LOG_TRIVIAL(info)
          << "Auto gain control: " << node.second.get<bool>("agc", false);
      BOOST_LOG_TRIVIAL(info) << "Gain: " << node.second.get<double>("gain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "IF Gain: " << node.second.get<double>("ifGain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "BB Gain: " << node.second.get<double>("bbGain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "LNA Gain: " << node.second.get<double>("lnaGain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "PGA Gain: " << node.second.get<double>("pgaGain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "TIA Gain: " << node.second.get<double>("tiaGain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "MIX Gain: " << node.second.get<double>("mixGain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "VGA1 Gain: " << node.second.get<double>("vga1Gain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "VGA2 Gain: " << node.second.get<double>("vga2Gain", 0);
      BOOST_LOG_TRIVIAL(info)
          << "Idle Silence: " << node.second.get<int>("silenceFrames", 0);
      BOOST_LOG_TRIVIAL(info) << "Digital Recorders: "
                              << node.second.get<int>("digitalRecorders", 0);
      BOOST_LOG_TRIVIAL(info)
          << "Debug Recorder: " << node.second.get<bool>("debugRecorder", 0);
      BOOST_LOG_TRIVIAL(info)
          << "SigMF Recorders: " << node.second.get<int>("sigmfRecorders", 0);
      BOOST_LOG_TRIVIAL(info)
          << "Analog Recorders: " << node.second.get<int>("analogRecorders", 0);

      if ((ppm != 0) && (error != 0)) {
        BOOST_LOG_TRIVIAL(info) << "Both PPM and Error should not be set at "
                                   "the same time. Setting Error to 0.";
        error = 0;
      }

      Source *source = new Source(center, rate, error, driver, device, &config);
      BOOST_LOG_TRIVIAL(info)
          << "Max Frequency: " << FormatFreq(source->get_max_hz());
      BOOST_LOG_TRIVIAL(info)
          << "Min Frequency: " << FormatFreq(source->get_min_hz());
      ...
      if (!gain_set) {
        BOOST_LOG_TRIVIAL(error)
            << "! No Gain was specified! Things will probably not work";
      }
    ...
    config.capture_dir = pt.get<std::string>(
        "captureDir", boost::filesystem::current_path().string());
    ...
    config.bcfy_calls_server =
        pt.get<std::string>("broadcastifyCallsServer", "");
    BOOST_LOG_TRIVIAL(info)
        << "Broadcastify Calls Server: " << config.bcfy_calls_server;
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Broadcast Signals: " << config.broadcast_signals;
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Call Timeout (seconds): " << config.call_timeout;
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Control channel decoding warnings (< messages/second): "
        << config.control_message_warn_rate;
    config.control_message_warn_updates =
        pt.get<float>("controlWarnUpdate", 3.0);
    BOOST_LOG_TRIVIAL(info) << "Control channel decoding updates (seconds): "
                            << config.control_message_warn_updates;
    config.control_retune_limit = pt.get<int>("controlRetuneLimit", 0);
    BOOST_LOG_TRIVIAL(info)
        << "Control channel retune limit: " << config.control_retune_limit;
  ...
  if (config.debug_recorder) {
    ...
    for (vector<Source *>::iterator it = sources.begin(); it != sources.end();
      ...
      Source *source = *it;
      BOOST_LOG_TRIVIAL(info)
          << "  " << source->get_driver() << " - " << source->get_device()
          << " [ " << FormatFreq(source->get_center())
          << " ]  Port: " << source->get_debug_recorder_port();
    }
    BOOST_LOG_TRIVIAL(info) << "\n\n-------------------------------------\n";
  }
  return true;
...
void process_signal(long unitId, const char *signaling_type,
...
bool start_recorder(Call *call, TrunkMessage message, System *sys) {
  ...
  if (!talkgroup && (sys->get_record_unknown() == false)) {
    if (sys->get_hideUnknown() == false) {
      BOOST_LOG_TRIVIAL(info)
          << "[" << sys->get_short_name()
          << "]\tTG: " << call->get_talkgroup_display()
          << "\tFreq: " << FormatFreq(call->get_freq())
          << "\t\u001b[33mNot Recording: TG not in Talkgroup File\u001b[0m ";
    }
    return false;
  }
  ...
  if (call->get_encrypted() == true ||
      (talkgroup && (talkgroup->mode == 'E'))) {
    if (sys->get_hideEncrypted() == false) {
      BOOST_LOG_TRIVIAL(info)
          << "[" << sys->get_short_name()
          << "]\tTG: " << call->get_talkgroup_display()
          << "\tFreq: " << FormatFreq(call->get_freq())
          << "\t\u001b[31mNot Recording: ENCRYPTED\u001b[0m ";
    }
    return false;
  }
  ...
  for (vector<Source *>::iterator it = sources.begin(); it != sources.end();
    ...
    if ((source->get_min_hz() <= call->get_freq()) &&
      ...
      if (talkgroup) {
        ...
        // Use an analog recorder if this is a Type II trunk and defaultMode is
        // analog. All other cases use a digital recorder.
        if ((default_mode == "analog") &&
            (sys->get_system_type() == "smartnet")) {
  ...
  if (!source_found) {
    BOOST_LOG_TRIVIAL(info)
        ...
        << "]\tTG: " << call->get_talkgroup_display()
        << "\tFreq: " << FormatFreq(call->get_freq())
        << "\t\u001b[36mNot Recording: no source covering Freq\u001b[0m";
    return false;
  }
  return false;
}
...
void process_message_queues() {
  for (std::vector<System *>::iterator it = systems.begin();
    ...
    for (std::vector<analog_recorder_sptr>::iterator arit =
             sys->conventional_recorders.begin();
         arit != sys->conventional_recorders.end(); ++arit) {
  ...
  for (vector<Call *>::iterator it = calls.begin(); it != calls.end();) {
    ...
    if (call->is_conventional() && call->get_recorder()) {
      ...
      if (call->get_current_length() > 0) {
        BOOST_LOG_TRIVIAL(trace) << "Recorder: " << call->get_current_length()
                                 << " Idle: " << call->get_recorder()->is_idle()
                                 << " Count: " << call->get_idle_count();
        ...
        // if no additional recording has happened in the past X periods, or the
        // call has gone on for longer than max_duration, stop and open new file
        if (call->get_idle_count() > 5) {
          ...
          if (recorder != NULL) {
            stats.send_recorder(recorder);
                       ...
                       call->get_system()->get_max_duration() &&
                   call->get_system()->get_max_duration() > 0) {
          BOOST_LOG_TRIVIAL(info)
              << "[" << call->get_short_name()
              << "]\t Restarting this call as it has a duration more than "
                 "maximum duration of "
              << call->get_system()->get_max_duration()
              << "\tTG: " << call->get_talkgroup_display()
              << "\tFreq: " << FormatFreq(call->get_freq())
              << "\tCall Duration: " << call->get_current_length() << "s";
          Recorder *recorder = call->get_recorder();
          call->end_call();
          stats.send_call_end(call);
          call->restart_call();
        ...
        // P25 Conventional Recorders need a have the graph unlocked before they
        // can start recording.
        Recorder *recorder = call->get_recorder();
        recorder->start(call);
        call->set_state(recording);
...
void print_status() {
  ...
  for (vector<Call *>::iterator it = calls.begin(); it != calls.end(); it++) {
    Call *call = *it;
    Recorder *recorder = call->get_recorder();
    BOOST_LOG_TRIVIAL(info) << "TG: " << call->get_talkgroup_display()
                            << " Freq: " << FormatFreq(call->get_freq())
                            << " Elapsed: " << call->elapsed()
                            << " State: " << FormatState(call->get_state());

    if (recorder) {
      BOOST_LOG_TRIVIAL(info)
          << "\t[ " << recorder->get_num()
          << " ] State: " << FormatState(recorder->get_state());
    }
  }
...
bool retune_recorder(TrunkMessage message, Call *call) {
  ...
  if (call->get_phase2_tdma() != message.phase2_tdma) {
    ...
    BOOST_LOG_TRIVIAL(info) << "\t - Message - \tTMDA: " << message.phase2_tdma
                            << " \tSlot: " << message.tdma_slot
                            << "\tCall - \tTMDA: " << call->get_phase2_tdma()
                            << "\tSlot: " << call->get_tdma_slot();
    BOOST_LOG_TRIVIAL(info)
        << "\t - Starting a new recording using a new recorder";
    return false;
  }
  ...
  if (message.freq != call->get_freq()) {
    if ((source->get_min_hz() <= message.freq) &&
      ...
      // TODO: Find the source that has the frequency on it, update our recorder
      // to use that source for the rest of the call
...
void unit_registration(string unit_script, string shortName, long unit) {
  ...
  if ((unit_script.length() != 0) && (unit != 0)) {
    ...
    sprintf(shell_command, "%s %s %li on &", unit_script.c_str(),
            shortName.c_str(), unit);
...
void unit_deregistration(string unit_script, string shortName, long unit) {
  ...
  if ((unit_script.length() != 0) && (unit != 0)) {
    char shell_command[200];
    sprintf(shell_command, "%s %s %li off &", unit_script.c_str(),
            shortName.c_str(), unit);
    int rc = system(shell_command);
  }
}
...
void unit_acknowledge(string unit_script, string shortName, long unit) {
  if ((unit_script.length() != 0) && (unit != 0)) {
    char shell_command[200];
    sprintf(shell_command, "%s %s %li ackresp &", unit_script.c_str(),
            shortName.c_str(), unit);
    int rc = system(shell_command);
  }
}

void unit_location_registration(string unit_script, string shortName, long unit,
                                long talkgroup) {
  if ((unit_script.length() != 0) && (unit != 0)) {
    char shell_command[200];
    sprintf(shell_command, "%s %s %li locreg %li &", unit_script.c_str(),
            shortName.c_str(), unit, talkgroup);
    int rc = system(shell_command);
  }
}

void group_affiliation(string unit_script, string shortName, long unit,
                       long talkgroup) {
  unit_affiliations[unit] = talkgroup;

  if ((unit_script.length() != 0) && (unit != 0)) {
    char shell_command[200];
    sprintf(shell_command, "%s %s %li join %li &", unit_script.c_str(),
            shortName.c_str(), unit, talkgroup);
    int rc = system(shell_command);
  }
}
...
void handle_call(TrunkMessage message, System *sys) {
  ...
  /* Notes: it is possible for 2 Calls to exist for the same talkgroup on
  different freq. This happens when a Talkgroup starts on a freq that current
  recorder can't retune to. In this case, the current orig Talkgroup reocrder
  will keep going on the old freq, while a new recorder is start on a source
  that can cover that freq. This makes sure any of the remaining transmission
  that it is in the buffer of the original recorder gets flushed. */

  unit_affiliations[message.source] = message.talkgroup;

  if ((sys->get_unit_script().length() != 0) && (message.source != 0)) {
    char shell_command[200];
    sprintf(shell_command, "%s %s %li call %li &",
            sys->get_unit_script().c_str(), sys->get_short_name().c_str(),
            message.source, message.talkgroup);
    int rc = system(shell_command);
  }
  ...
  for (vector<Call *>::iterator it = calls.begin(); it != calls.end();) {
    ...
    if ((call->get_talkgroup() == message.source) &&
        (call->get_sys_num() == message.sys_num)) {
      BOOST_LOG_TRIVIAL(error)
          << "WARNING: Old call found w/this message.source. Old TG: "
          << call->get_talkgroup_display() << "/" << call->get_talkgroup()
          << "New TG: " << message.talkgroup << " New src: " << message.source;
    }
    ...
    if ((call->get_talkgroup() == message.talkgroup) &&
      ...
      // Check to make sure the Freq and TDMA info match up with what is being
      // currenty recorded
      if ((call->get_freq() != message.freq) ||
          (call->get_tdma_slot() != message.tdma_slot) ||
          (call->get_phase2_tdma() != message.phase2_tdma)) {
        if (call->get_state() == recording) {
          // see if we can retune the recorder, You may not be able to if the
          // Freq is beyond what the current source can handle
          int retuned = retune_recorder(message, call);

          if (!retuned) {
            // we want to keep this call recording and now start a recording of
            // the new call on another recorder
            call_found = false;
            retune_failed = true;
            ...
            BOOST_LOG_TRIVIAL(info)
                ...
                << "]\tTG: " << call->get_talkgroup_display()
                << "\tFreq: " << FormatFreq(call->get_freq())
                << "\tUpdate Retuning - New Freq: " << FormatFreq(message.freq)
                << "\tElapsed: " << call->elapsed()
                << "s \tSince update: " << call->since_last_update() << "s";
            call->update(message);
            call_retune = true;
            break;
          }
        ...
        // everything about the current recording matches, simply update the
        // info
        call->update(message);
        break;
      }
      ...
      ++it;
      // the talkgroup for the call being looked at doesn't match, look at the
      // next one.
    }
  }
  ...
  if (!call_found) {
    if (retune_failed) {
      BOOST_LOG_TRIVIAL(info)
          << "\t - Retune failed, starting a new recording using a new source";
    }
...
void unit_check() {
  ...
  if (myfile.is_open()) {
    ...
    // sprintf(shell_command, "./unit_check.sh %s > /dev/null 2>&1 &",
    // unit_filename); int rc = system(shell_command);
    myfile << "}";
    myfile.close();
  }
}
...
void handle_message(std::vector<TrunkMessage> messages, System *sys) {
  for (std::vector<TrunkMessage>::iterator it = messages.begin();
       it != messages.end(); it++) {
    ...
    switch (message.message_type) {
      ...
      case REGISTRATION:
        unit_registration(sys->get_unit_script(), sys->get_short_name(),
                          message.source);
        break;

      case DEREGISTRATION:
        unit_deregistration(sys->get_unit_script(), sys->get_short_name(),
                            message.source);
        break;

      case AFFILIATION:
        group_affiliation(sys->get_unit_script(), sys->get_short_name(),
                          message.source, message.talkgroup);
        break;
      ...
      case ACKNOWLEDGE:
        unit_acknowledge(sys->get_unit_script(), sys->get_short_name(),
                         message.source);
        break;

      case LOCATION:
        unit_location_registration(sys->get_unit_script(),
                                   sys->get_short_name(), message.source,
                                   message.talkgroup);
        break;
...
void retune_system(System *system) {
  ...
  BOOST_LOG_TRIVIAL(error) << "[" << system->get_short_name()
                           ...
                           << FormatFreq(control_channel_freq);

  if ((current_source->get_min_hz() <= control_channel_freq) &&
      (current_source->get_max_hz() >= control_channel_freq)) {
    source_found = true;
    BOOST_LOG_TRIVIAL(info)
        << "\t - System Source " << current_source->get_num()
        << " - Min Freq: " << FormatFreq(current_source->get_min_hz())
        << " Max Freq: " << FormatFreq(current_source->get_max_hz());
    ...
    for (vector<Source *>::iterator src_it = sources.begin();
      ...
      if ((source->get_min_hz() <= control_channel_freq) &&
        ...
        source_found = true;
        BOOST_LOG_TRIVIAL(info)
            << "\t - System Source " << source->get_num()
            << " - Min Freq: " << FormatFreq(source->get_min_hz())
            << " Max Freq: " << FormatFreq(source->get_max_hz());

        if (system->get_system_type() == "smartnet") {
          system->set_source(source);
          // We must lock the flow graph in order to disconnect and reconnect
          // blocks
          tb->lock();
          tb->disconnect(current_source->get_src_block(), 0,
                         system->smartnet_trunking, 0);
  ...
  if (!source_found) {
    BOOST_LOG_TRIVIAL(error) << "\t - Unable to retune System control channel, "
                                "freq not covered by any source.";
  }
}
...
void check_message_count(float timeDiff) {
  ...
  for (std::vector<System *>::iterator it = systems.begin();
    ...
    if ((sys->system_type != "conventional") &&
        (sys->system_type != "conventionalP25")) {
      ...
      if (msgs_decoded_per_second < 2) {
        ...
        if (sys->control_channel_count() > 1) {
          ...
          BOOST_LOG_TRIVIAL(error)
              << "[" << sys->get_short_name()
              << "]\tThere is only one control channel defined";
        }
    ...
    if (msg != 0) {
      ...
      if (sys) {
        ...
        if (sys->get_system_type() == "smartnet") {
          trunk_messages =
              smartnet_parser->parse_message(msg->to_string(), sys);
      ...
      if (msg->type() == -1) {
        BOOST_LOG_TRIVIAL(error)
            << "[" << sys->get_short_name() << "]\t process_data_unit timeout";
      }
    ...
    if ((system->get_system_type() == "conventional") ||
          ...
          if ((source->get_min_hz() <= channel) &&
            ...
            if (system->get_squelch_db() == -160) {
              BOOST_LOG_TRIVIAL(error)
                  ...
                  << "]\tSquelch needs to be specified for the Source for "
            ...
            Call_conventional *call = new Call_conventional(
                tg_iterate_index, channel, system, config);
            Talkgroup *talkgroup =
            ...
            if (system->get_system_type() == "conventional") {
              ...
              // Because of dynamic mod assignment we can not start the recorder
              // until the graph has been unlocked. This has something to do
              // with the way the Selector block works. the
              // stop_inactive_recorders() function handles adding and starting
              // the P25 Recorder
        ...
        if (!channel_added) {
          BOOST_LOG_TRIVIAL(error)
              ...
              << "]\t Unable to find a source for this conventional channel! "
                 "Channel not added: "
              << FormatFreq(channel) << " Talkgroup: " << tg_iterate_index;
        }
      }
    } else {
      ...
      for (vector<Source *>::iterator src_it = sources.begin();
        ...
        if ((source->get_min_hz() <= control_channel_freq) &&
          ...
          if (system->get_system_type() == "smartnet") {
            // what you really need to do is go through all of the sources to
            // find the one with the right frequencies
            system->smartnet_trunking = make_smartnet_trunking(
                control_channel_freq, source->get_center(), source->get_rate(),
                msg_queue, system->get_sys_num());
            tb->connect(source->get_src_block(), 0, system->smartnet_trunking,
                        0);
          }
          ...
          if (system->get_system_type() == "p25") {
            ...
            system->p25_trunking = make_p25_trunking(
                control_channel_freq, source->get_center(), source->get_rate(),
                msg_queue, system->get_qpsk_mod(), system->get_sys_num());
            tb->connect(source->get_src_block(), 0, system->p25_trunking, 0);
          }
...
void add_logs(const F &fmt) {
  boost::shared_ptr<
      sinks::synchronous_sink<sinks::basic_text_ostream_backend<char>>>
      sink = boost::log::add_console_log(std::clog,
...
void socket_connected() {
  ...
  for (vector<Source *>::iterator it = sources.begin(); it != sources.end();
    ...
    recorders.insert(recorders.end(), sourceRecorders.begin(),
                     sourceRecorders.end());
  }
...
int main(int argc, char **argv) {
  ...
  add_logs(boost::log::expressions::format("[%1%] (%2%)   %3%") %
  ...
  desc.add_options()("help,h", "Help screen")(
      "config",
      boost::program_options::value<string>()->default_value("./config.json"),
      "Config File");
  ...
  if (monitor_system()) {
    ...
    BOOST_LOG_TRIVIAL(error)
        << "Unable to setup a System to record, exiting..." << std::endl;
  }
