...
p25_recorder::DecimSettings p25_recorder::get_decim(long speed) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "P25 recorder Decim: " << decim_settings.decim
                             << " Decim2:  " << decim_settings.decim2;
...
void p25_recorder::initialize_prefilter() {
  ...
  // double phase2_channel_rate = phase2_symbol_rate *
  // phase2_samples_per_symbol;
  ...
  lo = gr::analog::sig_source_c::make(input_rate, gr::analog::GR_SIN_WAVE, 0,
                                      1.0, 0.0);
  ...
  if (decim_settings.decim != -1) {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "\t P25 Recorder two-stage decimator - Initial decimated rate: "
        << if1 << " Second decimated rate: " << if2 << " FA: " << fa
        << " FB: " << fb << " System Rate: " << input_rate;
    bandpass_filter_coeffs = gr::filter::firdes::complex_band_pass(
        1.0, input_rate, -if1 / 2, if1 / 2, if1 / 2);
    lowpass_filter_coeffs =
        gr::filter::firdes::low_pass(1.0, if1, (fb + fa) / 2, fb - fa);
    bandpass_filter = gr::filter::fft_filter_ccc::make(decim_settings.decim,
                                                       bandpass_filter_coeffs);
    lowpass_filter = gr::filter::fft_filter_ccf::make(decim_settings.decim2,
                                                      lowpass_filter_coeffs);
    resampled_rate = if2;
    bfo = gr::analog::sig_source_c::make(if1, gr::analog::GR_SIN_WAVE, 0, 1.0,
                                         0.0);
  } else {
    double_decim = false;
    BOOST_LOG_TRIVIAL(info)
        << "\t P25 Recorder single-stage decimator - Initial decimated rate: "
        << if1 << " Second decimated rate: " << if2
        << " Initial Decimation: " << decim << " System Rate: " << input_rate;
    lo = gr::analog::sig_source_c::make(input_rate, gr::analog::GR_SIN_WAVE, 0,
                                        1.0, 0.0);
    lowpass_filter_coeffs =
        gr::filter::firdes::low_pass(1.0, input_rate, 7250, 1450);
    decim = floor(input_rate / if_rate);
    resampled_rate = input_rate / decim;

    lowpass_filter =
        gr::filter::fft_filter_ccf::make(decim, lowpass_filter_coeffs);
    resampled_rate = input_rate / decim;
  }
  ...
  fa = 6250;
  fb = fa + 625;
  cutoff_filter_coeffs =
      gr::filter::firdes::low_pass(1.0, if_rate, (fb + fa) / 2, fb - fa);
  ...
  BOOST_LOG_TRIVIAL(info) << "\t P25 Recorder ARB - Initial Rate: "
                          << input_rate << " Resampled Rate: " << resampled_rate
                          << " Initial Decimation: " << decim
                          << " System Rate: " << system_channel_rate
                          << " ARB Rate: " << arb_rate;
...
void p25_recorder::switch_tdma(bool phase2) {
  ...
  BOOST_LOG_TRIVIAL(info) << "\t P25 Recorder - Adjust Channel Resampled Rate: "
                          << resampled_rate
                          << " System Rate: " << system_channel_rate
                          << " ARB Rate: " << arb_rate;
...
void p25_recorder::autotune() {
  /*if (!qpsk_mod) {
    ...
    if (msg != 0) {
      BOOST_LOG_TRIVIAL(info) << "p25_recorder.cc: Freq:\t" <<
  FormatFreq(chan_freq) << "\t Tune: " << msg->arg1();
...
Source *p25_recorder::get_source() { return source; }

int p25_recorder::get_num() { return rec_num; }
...
double p25_recorder::get_freq() { return chan_freq; }
...
int p25_recorder::lastupdate() { return time(NULL) - timestamp; }

long p25_recorder::elapsed() { return time(NULL) - starttime; }
...
void p25_recorder::tune_offset(double f) {
  ...
  if (abs(freq) > ((input_rate / 2) - (if1 / 2))) {
    BOOST_LOG_TRIVIAL(info)
        << "Tune Offset: Freq exceeds limit: " << abs(freq)
        << " compared to: " << ((input_rate / 2) - (if1 / 2));
  }
  if (double_decim) {
    bandpass_filter_coeffs = gr::filter::firdes::complex_band_pass(
        1.0, input_rate, -freq - if1 / 2, -freq + if1 / 2, if1 / 2);
...
State p25_recorder::get_state() { return state; }
...
/* This is called by wav_sink_delayed_open to get a new filename when samples
 * start to come in */
...
void p25_recorder::stop() {
  if (state == active) {
    ...
    clear();
    BOOST_LOG_TRIVIAL(info)
        << "\t- Stopping P25 Recorder Num [" << rec_num
        << "]\tTG: " << this->call->get_talkgroup_display()
        << "\tFreq: " << FormatFreq(chan_freq) << " \tTDMA: " << d_phase2_tdma
        << "\tSlot: " << tdma_slot;
    ...
    BOOST_LOG_TRIVIAL(error)
        << "p25_recorder.cc: Trying to Stop an Inactive Logger!!!";
  }
}
...
void p25_recorder::start(Call *call) {
  if (state == inactive) {
    ...
    if (call->get_phase2_tdma()) {
      if (!qpsk_mod) {
        BOOST_LOG_TRIVIAL(error) << "Error - Modulation is FSK4 but receiving "
                                    "Phase 2 call, this will not work";
      }
    ...
    /*
        if (d_delayopen) {
          BOOST_LOG_TRIVIAL(info) << "\t- Listening P25 Recorder Num [" <<
       rec_num << "]\tTG: " << this->call->get_talkgroup_display() << "\tFreq: "
       << FormatFreq(chan_freq) << " \tTDMA: " << call->get_phase2_tdma() <<
       "\tSlot: " << call->get_tdma_slot(); } else { recording_started();
        }
    */

    BOOST_LOG_TRIVIAL(info)
        << "\t- Starting P25 Recorder Num [" << rec_num
        << "]\tTG: " << this->call->get_talkgroup_display()
        << "\tFreq: " << FormatFreq(chan_freq)
        << " \tTDMA: " << call->get_phase2_tdma()
        << "\tSlot: " << call->get_tdma_slot() << "\tMod: " << qpsk_mod;
    ...
    BOOST_LOG_TRIVIAL(error)
        << "p25_recorder.cc: Trying to Start an already Active Logger!!!";
  }
}
