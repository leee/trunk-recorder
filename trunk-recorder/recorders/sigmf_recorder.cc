...
sigmf_recorder::sigmf_recorder(Source *src)
  ...
  int nchars =
      snprintf(filename, 160, "%ld-%ld_%g.raw", talkgroup, starttime, freq);

  if (nchars >= 160) {
    BOOST_LOG_TRIVIAL(error)
        << "Analog Recorder: Path longer than 160 charecters";
  }
...
long sigmf_recorder::get_source_count() { return 0; }

Call_Source *sigmf_recorder::get_source_list() {
  return NULL;  // wav_sink->get_source_list();
}

Source *sigmf_recorder::get_source() { return source; }

int sigmf_recorder::get_num() { return rec_num; }
...
double sigmf_recorder::get_freq() { return freq; }

double sigmf_recorder::get_current_length() {
  return 0;  // wav_sink->length_in_seconds();
}

int sigmf_recorder::lastupdate() { return time(NULL) - timestamp; }

long sigmf_recorder::elapsed() { return time(NULL) - starttime; }
...
State sigmf_recorder::get_state() { return state; }

void sigmf_recorder::stop() {
  if (state == active) {
    recording_duration += wav_sink->length_in_seconds();
    BOOST_LOG_TRIVIAL(error)
        << "sigmf_recorder.cc: Stopping Logger \t[ " << rec_num << " ] - freq[ "
        << freq << "] \t talkgroup[ " << talkgroup << " ]";
    state = inactive;
    valve->set_enabled(false);
    raw_sink->close();
  } else {
    BOOST_LOG_TRIVIAL(error)
        << "sigmf_recorder.cc: Trying to Stop an Inactive Logger!!!";
  }
}
...
void sigmf_recorder::start(Call *call) {
  if (state == inactive) {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "sigmf_recorder.cc: Starting Logger   \t[ " << rec_num
        << " ] - freq[ " << freq << "] \t talkgroup[ " << talkgroup << " ]";
    ...
    BOOST_LOG_TRIVIAL(error)
        << "sigmf_recorder.cc: Trying to Start an already Active Logger!!!";
  }
}
