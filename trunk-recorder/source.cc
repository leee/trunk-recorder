...
void Source::set_antenna(std::string ant) {
  ...
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Setting antenna to ["
        << cast_to_osmo_sptr(source_block)->get_antenna() << "]";
  }
...
void Source::set_silence_frames(int m) { silence_frames = m; }

int Source::get_silence_frames() { return silence_frames; }
...
void Source::set_mix_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info) << "MIX Gain set to: "
                            << cast_to_osmo_sptr(source_block)->get_gain("MIX");
  }
}
  ...
  if (driver == "osmosdr") {
    try {
      ...
      BOOST_LOG_TRIVIAL(error)
          << "MIX Gain unsupported or other error: " << e.what();
    }
  }
  return mix_gain;
}
...
void Source::set_lna_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info) << "LNA Gain set to: "
                            << cast_to_osmo_sptr(source_block)->get_gain("LNA");
  }
}
...
int Source::get_lna_gain() {
  if (driver == "osmosdr") {
    try {
      ...
      BOOST_LOG_TRIVIAL(error)
          << "LNA Gain unsupported or other error: " << e.what();
    }
  }
  return lna_gain;
}
...
void Source::set_tia_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info) << "TIA Gain set to: "
                            << cast_to_osmo_sptr(source_block)->get_gain("TIA");
  }
}
...
int Source::get_tia_gain() {
  if (driver == "osmosdr") {
    try {
      ...
      BOOST_LOG_TRIVIAL(error)
          << "TIA Gain unsupported or other error: " << e.what();
    }
  }
  return tia_gain;
}
...
void Source::set_pga_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info) << "PGA Gain set to: "
                            << cast_to_osmo_sptr(source_block)->get_gain("PGA");
  }
}
...
int Source::get_pga_gain() {
  if (driver == "osmosdr") {
    try {
      ...
      BOOST_LOG_TRIVIAL(error)
          << "PGA Gain unsupported or other error: " << e.what();
    }
  }
  return pga_gain;
}
...
void Source::set_vga1_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "VGA1 Gain set to: "
        << cast_to_osmo_sptr(source_block)->get_gain("VGA1");
  }
}
...
int Source::get_vga1_gain() {
  if (driver == "osmosdr") {
    try {
      ...
      BOOST_LOG_TRIVIAL(error)
          << "VGA1 Gain unsupported or other error: " << e.what();
    }
  }
  return vga1_gain;
}
...
void Source::set_vga2_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "VGA2 Gain set to: "
        << cast_to_osmo_sptr(source_block)->get_gain("VGA2");
  }
}
...
int Source::get_vga2_gain() {
  if (driver == "osmosdr") {
    try {
      ...
      BOOST_LOG_TRIVIAL(error)
          << "VGA2 Gain unsupported or other error: " << e.what();
    }
  }
  return vga2_gain;
}
...
void Source::set_bb_gain(int b) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info) << "BB Gain set to: "
                            << cast_to_osmo_sptr(source_block)->get_gain("BB");
  }
}
...
void Source::set_gain(int r) {
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Gain set to: " << cast_to_osmo_sptr(source_block)->get_gain();
  }
...
bool Source::get_gain_mode() { return gain_mode; }

void Source::set_if_gain(int i) {
  if (driver == "osmosdr") {
    if_gain = i;
    cast_to_osmo_sptr(source_block)->set_if_gain(if_gain);
    BOOST_LOG_TRIVIAL(info) << "IF Gain set to: "
                            << cast_to_osmo_sptr(source_block)->get_gain("IF");
  }
}
...
void Source::set_freq_corr(double p) {
  ...
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "PPM set to: " << cast_to_osmo_sptr(source_block)->get_freq_corr();
  }
}

int Source::get_if_gain() { return if_gain; }

analog_recorder_sptr Source::create_conventional_recorder(
...
Recorder *Source::get_analog_recorder(Talkgroup *talkgroup) {
  ...
  if (talkgroup &&
      talkgroup->get_priority() >
          num_available_recorders) {  // a low priority is bad. You need atleast
                                      // the number of availalbe recorders to
                                      // your priority
    BOOST_LOG_TRIVIAL(info)
        << "\t\tNot recording talkgroup " << talkgroup->number << " ("
        << talkgroup->alpha_tag << ")"
        << ", priority is " << talkgroup->get_priority() << " but only "
        << num_available_recorders << " recorders available";
    return NULL;
  }
...
Recorder *Source::get_analog_recorder() {
  for (std::vector<analog_recorder_sptr>::iterator it =
           analog_recorders.begin();
...
p25_recorder_sptr Source::create_digital_conventional_recorder(
    gr::top_block_sptr tb) {
  // Not adding it to the vector of digital_recorders. We don't want it to be
  // available for trunk recording.
  p25_recorder_sptr log = make_p25_recorder(this);
  tb->connect(source_block, 0, log, 0);
  return log;
}
...
void Source::create_debug_recorder(gr::top_block_sptr tb, int source_num) {
  ...
  debug_recorder_sptr log = make_debug_recorder(
      this, config->debug_recorder_address, debug_recorder_port);
...
int Source::get_debug_recorder_port() { return debug_recorder_port; }
...
void Source::print_recorders() {
  ...
  for (std::vector<p25_recorder_sptr>::iterator it = digital_recorders.begin();
    ...
    BOOST_LOG_TRIVIAL(info) << "[ D" << rx->get_num()
                            << " ] State: " << FormatState(rx->get_state());
  }
  ...
  for (std::vector<analog_recorder_sptr>::iterator it =
    ...
    BOOST_LOG_TRIVIAL(info) << "[ A" << rx->get_num()
                            << " ] State: " << FormatState(rx->get_state());
  }
}
...
int Source::digital_recorder_count() { return digital_recorders.size(); }

int Source::analog_recorder_count() { return analog_recorders.size(); }

int Source::debug_recorder_count() { return debug_recorders.size(); }

int Source::sigmf_recorder_count() { return sigmf_recorders.size(); }

int Source::get_num() { return src_num; };
...
Recorder *Source::get_digital_recorder() {
  ...
  for (std::vector<p25_recorder_sptr>::iterator it = digital_recorders.begin();
    ...
    p25_recorder_sptr rx = *it;
    BOOST_LOG_TRIVIAL(info)
        << "[ " << rx->get_num() << " ] State: " << FormatState(rx->get_state())
        << " Freq: " << rx->get_freq();
  }
  return NULL;
}
...
Config *Source::get_config() { return config; }
...
Source::Source(double c, double r, double e, std::string drv, std::string dev,
  ...
  if (driver == "osmosdr") {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Setting sample rate to: " << FormatSamplingRate(rate);
    osmo_src->set_sample_rate(rate);
    actual_rate = osmo_src->get_sample_rate();
    BOOST_LOG_TRIVIAL(info)
        << "Actual sample rate: " << FormatSamplingRate(actual_rate);
    ...
    for (std::vector<std::string>::iterator it = gain_names.begin();
         it != gain_names.end(); it++) {
      ...
      for (std::vector<double>::iterator gain_it = gains.begin();
           gain_it != gains.end(); gain_it++) {
