...
p25_trunking_sptr make_p25_trunking(double freq, double center, long s,
                                    gr::msg_queue::sptr queue, bool qpsk,
                                    int sys_num) {
  return gnuradio::get_initial_sptr(
      new p25_trunking(freq, center, s, queue, qpsk, sys_num));
}
...
p25_trunking::DecimSettings p25_trunking::get_decim(long speed) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "P25 trunking Decim: " << decim_settings.decim
...
void p25_trunking::initialize_prefilter() {
  ...
  if (decim_settings.decim != -1) {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "\t P25 Trunking two-stage decimator - Initial decimated rate: "
    ...
    double_decim = false;
    BOOST_LOG_TRIVIAL(info)
        << "\t P25 Trunking single-stage decimator - Initial decimated rate: "
  ...
  BOOST_LOG_TRIVIAL(info) << "\t P25 Trunking ARB - Initial Rate: "
...
void p25_trunking::initialize_qpsk() {
  ...
  double omega = double(system_channel_rate) /
                 symbol_rate;  // set to 6000 for TDMA, should be symbol_rate
  ...
  costas_clock = gr::op25_repeater::gardner_costas_cc::make(
      omega, gain_mu, gain_omega, alpha, beta, fmax, -fmax);
...
void p25_trunking::initialize_p25() {
  ...
  bool do_tdma = 0;
  bool do_crypt = 0;
  op25_frame_assembler = gr::op25_repeater::p25_frame_assembler::make(
      sys_num, silence_frames, wireshark_host, udp_port, verbosity, do_imbe,
      do_output, do_msgq, rx_queue, do_audio_output, do_tdma, do_crypt);

  connect(slicer, 0, op25_frame_assembler, 0);
}

p25_trunking::p25_trunking(double f, double c, long s,
                           gr::msg_queue::sptr queue, bool qpsk, int sys_num)
...
void p25_trunking::set_center(double c) { center_freq = c; }
...
double p25_trunking::get_freq() { return chan_freq; }
