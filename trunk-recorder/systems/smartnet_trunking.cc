...
smartnet_trunking_sptr make_smartnet_trunking(float freq, float center,
                                              long samp,
                                              gr::msg_queue::sptr queue,
                                              int sys_num) {
  return gnuradio::get_initial_sptr(
      new smartnet_trunking(freq, center, samp, queue, sys_num));
}
...
smartnet_trunking::DecimSettings smartnet_trunking::get_decim(long speed) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    BOOST_LOG_TRIVIAL(debug)
        << "SmartNet trunking Decim: " << decim_settings.decim
...
void smartnet_trunking::initialize_prefilter() {
  ...
  if (decim_settings.decim != -1) {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "\t smartnet Trunking two-stage decimator - Initial decimated rate: "
    ...
    double_decim = false;
    BOOST_LOG_TRIVIAL(info)
        << "\t smartnet Trunking single-stage decimator - Initial decimated "
           "rate: "
  ...
  BOOST_LOG_TRIVIAL(info) << "\t smartnet Trunking ARB - Initial Rate: "
...
smartnet_trunking::smartnet_trunking(float f, float c, long s,
  ...
  carriertrack =
      gr::digital::fll_band_edge_cc::make(system_channel_rate, 0.6, 64, 0.35);

  pll_demod = gr::analog::pll_freqdet_cf::make(2.0 / samples_per_symbol,
                                               1 * pi / samples_per_symbol,
                                               -1 * pi / samples_per_symbol);

  softbits = gr::digital::clock_recovery_mm_ff::make(
      samples_per_symbol, 0.25 * gain_mu * gain_mu, mu, gain_mu,
      omega_relative_limit);

  slicer = gr::digital::binary_slicer_fb::make();

  start_correlator = gr::digital::correlate_access_code_tag_bb::make(
      "10101100", 0, "smartnet_preamble");
...
void smartnet_trunking::reset() {
  BOOST_LOG_TRIVIAL(info) << "Pll Phase: " << pll_demod->get_phase()
                          << " min Freq: "
                          << FormatFreq(pll_demod->get_min_freq())
                          << " Max Freq: "
                          << FormatFreq(pll_demod->get_max_freq());
...
void smartnet_trunking::set_center(double c) { center_freq = c; }
